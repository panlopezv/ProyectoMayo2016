...
public class modeloProductos extends AbstractTableModel {
    ...
    public modeloProductos(ArrayList<Producto> productos, int opc) {
        if (opc == 0) {
            columnas = new String[]{"Código", "Producto", "Categoría", "Precio", "Existencias"};
        } else {
            columnas = new String[]{"Código", "Producto", "Categoría", "Costo actual", "Existencias"};
        }
            ...
            if (i < productos.size() && i1 < columnas.length) {
                Producto p = productos.get(i);
                    switch (i1) {
                        ...
                        case 3:
                            return (new DecimalFormat("Q#,##0.00")).format(p.getPrecio());
                        case 4:
