...
public class DetalleCompra extends javax.swing.JPanel {
    ...
    public DetalleCompra(int idCompra) {
        initComponents();
        Query q = Conexion.getConexion().getEmf().createEntityManager().createNamedQuery("Detallecompra.findByIdCompra");
        q.setParameter("idCompra", idCompra);
        try {
            modeloDC = new ModeloDetalleCompra(new ArrayList<>(q.getResultList()));
        } catch (Exception ex) {
            modeloDC = new ModeloDetalleCompra(new ArrayList<>());
        }
        jTable1.setModel(modeloDC);
